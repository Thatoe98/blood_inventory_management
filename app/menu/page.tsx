...
export default function MenuPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-red-50/20 to-blue-50/20">
      {/* Header */}
      <div className="bg-gradient-to-r from-red-600 via-red-500 to-pink-500 text-white py-4 px-6 shadow-xl">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">Blood Bank System</h1>
                <p className="text-red-100 text-sm">Main Menu - Admin Portal</p>
              </div>
            </div>
            <button
              onClick={handleLogout}
              className="px-4 py-2 bg-white/20 backdrop-blur-sm hover:bg-white/30 rounded-lg font-semibold transition-all duration-300 flex items-center gap-2 border border-white/30 text-sm"
            >
              <span>🚪</span>
              Logout
            </button>
      ...
      <div className="max-w-7xl mx-auto p-4">
        {/* Welcome Message */}
        <div className="bg-white rounded-xl shadow-lg p-4 mb-4 border-l-4 border-red-500">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-red-500 to-pink-500 rounded-full flex items-center justify-center">
              <span className="text-xl">👋</span>
            </div>
            <div>
              <h2 className="text-xl font-bold text-gray-800">Welcome, Administrator!</h2>
              <p className="text-gray-600 text-sm">Choose an option below to manage the blood bank system</p>
            </div>
          </div>
        </div>
          ...
          <div>
            <div className="bg-white rounded-xl shadow-lg p-4 h-full">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b-2 border-gray-200">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-800">View</h3>
                ...
                {viewItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="relative flex flex-col items-center">
                      <div className={`w-8 h-8 bg-gradient-to-br ${item.color} rounded-lg flex items-center justify-center text-base shadow-sm group-hover:scale-110 transition-transform duration-300 mb-1`}>
                        {item.icon}
                      </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {actionItems.map((item) => (
                  ...
                  >
                    <div className={`absolute inset-0 bg-gradient-to-r ${item.color} opacity-0 group-hover:opacity-10 transition-opacity duration-300`}></div>
                    <div className="relative flex flex-col items-center">
                      <div className={`w-8 h-8 bg-gradient-to-br ${item.color} rounded-lg flex items-center justify-center text-sm shadow-sm group-hover:scale-110 transition-transform duration-300 mb-1`}>
                        {item.icon}
                      </div>
                      <h4 className="font-semibold text-xs text-gray-800 group-hover:text-gray-900 text-center">{item.name}</h4>
                    </div>
